...
- 文档标题优先用 `--title`。CLI 会把它补成内容开头的 `<title>`；如果内容里又出现 `<title>`，SDK 会保留第一个标题并过滤后续标题，同时在 `warnings` / `degrade_details` 中提示。
- XML 内容也可以直接写 `<title>`；但不要同时传 `--title` 和手写另一个 `<title>`，除非你明确接受后者被过滤。
- Markdown 创建时用 `--title` 指定文档标题，不再要求正文开头必须是唯一一级标题；正文 heading 按内容自然组织即可。
- **创建较长的文档时只建骨架**：用 `--title` 传文档标题，`--content` 仅传各级 heading + 简短占位摘要；正文留给后续 `block_insert_after --block-id <章节标题 block_id>` 分段追加。一次性塞超长 `--content` 既容易触发参数限制，调试也更难。
