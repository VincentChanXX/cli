...
func TestEnsureInitDirMatchesApp(t *testing.T) {
	...
	if err := ensureInitDirMatchesApp(t.TempDir(), "app_x"); err != nil {
		t.Fatalf("no meta should pass: %v", err)
	}
	// 同 app_id → nil（走已初始化短路）
	if err := ensureInitDirMatchesApp(writeMeta(t, `{"app_id":"app_x"}`), "app_x"); err != nil {
		t.Fatalf("same app should pass: %v", err)
	}

	// 不同 app_id → error（换目录）；断言 typed metadata（category/subtype/param），不只看子串
	errMismatch := ensureInitDirMatchesApp(writeMeta(t, `{"app_id":"app_other"}`), "app_x")
	if errMismatch == nil {
	...
	// 空 app_id（无法确认一致）→ error（换目录），按 mismatch 处理
	errEmpty := ensureInitDirMatchesApp(writeMeta(t, `{"name":"x"}`), "app_x")
	if errEmpty == nil {
	...
	if !strings.Contains(pEmpty.Message, "different app") {
		t.Fatalf("empty app_id should be treated as mismatch, msg=%q", pEmpty.Message)
	}

	// meta 损坏/不可读 → error（fail closed）
	errBad := ensureInitDirMatchesApp(writeMeta(t, `{not json`), "app_x")
	if errBad == nil {
